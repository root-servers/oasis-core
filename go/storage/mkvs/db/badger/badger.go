	...
	for it.Rewind(); it.Valid(); it.Next() {
		...
		if removeNodes {
			...
			case node.RootTypeInvalid:
				h := hash.Hash()
				if err := nodeBatch.Delete(nodeKeyFmt.Encode(&h)); err != nil {
					return err
				}
			default:
				if err := nodeBatch.Delete(rootNodeKeyFmt.Encode(&hash)); err != nil {
					return err
				}
			}
		}
...
func (s *badgerSubtree) PutNode(depth node.Depth, ptr *node.Pointer) error {
	...
	if s.batch.multipartNodes != nil {
		if _, err = s.batch.readTxn.Get(nodeKey); err != nil && errors.Is(err, badger.ErrKeyNotFound) {
			...
			if depth == 0 {
				th = typedHashFromParts(s.batch.oldRoot.Type, h)
				if err = s.batch.multipartNodes.Set(multipartRestoreNodeLogKeyFmt.Encode(&th), []byte{}); err != nil {
					return err
				}
			}
	...
	if depth == 0 {
		th := typedHashFromParts(s.batch.oldRoot.Type, h)
		if err = s.batch.bat.Set(rootNodeKeyFmt.Encode(&th), []byte{}); err != nil {
			return err
		}
	}
	return nil
